...
app.mount("/profchatile_pictures", StaticFiles(directory="profile_pictures"), name="profile_pictures")
app.mount("/statchatic", StaticFiles(directory="static"), name="static")
chat
# Создаем сессию chat для взаимодействия с базой данных
engichatne = create_chatasync_engine("mysql+aiomysql://root:root@localhost/chat")
...
async def end_poll(chat_id: int, poll_id: int):
...
async def read_chat(request: Request, chat_id: int, current_user: Union[str, RedirectResponse] = Depends(get_current_user)):
    ...
    for message in messages:
        if message.message.startswith('Опрос: '):
            poll_id = int(message.message.split(' ')[1])
