...
# Third-party libraries for web frameworks, security, and templating
from fastapi import FastAPI, Request, WebSocket, HTTPException, Depends, status, Form, Cookie, logger, UploadFile, File
...
# SQLAlchemy modules for working with SQL databases
from sqlalchemy import create_engine, MetaData, Column, Integer, String, ForeignKey, PrimaryKeyConstraint, DateTime, \
    Table, func, LargeBinary, desc, or_, Boolean, BLOB, Text
from sqlalchemy.exc import SQLAlchemyError, NoResultFound
from sqlalchemy.ext.declarative import declarative_base
from sqlalchemy.orm import sessionmaker
from sqlalchemy.sql import select

# Modules for handling SQL database connections
from databases import Database
from pymysql.err import ProgrammingError
from django.core.checks import database


# Security and password hashing modules
import bcrypt
...
# Устанавливаем метаданные, шаблоны и базовый класс для моделей SQLAlchemy
...
async def update_user_nickname(phone_number: str, nickname: str):
    await update_user_field(phone_number, "nickname", nickname)

async def update_user_status(phone_number: str, status: str):
    await update_user_field(phone_number, "status", status)

async def update_user_profile_picture(phone_number: str, profile_picture: str):
    await update_user_field(phone_number, "profile_picture", profile_picture)

# Папка, где будут храниться фотографии профиля
PROFILE_PICTURES_FOLDER = "profile_pictures"  # здесь можно изменить путь к папке для хранения изображений
...
async def update_user_profile(request: Request,
    ...
    if nickname:
        ...
        await update_user_nickname(phone_number, nickname)  # Изменил здесь
    if status:
        # обновляем статус пользователя
        await update_user_status(phone_number, status)  # Изменил здесь
    if profile_picture:
        # загружаем новую фотографию профиля
        picture_path = await save_profile_picture(profile_picture)
        await update_user_profile_picture(phone_number, picture_path)  # Изменил здесь
    # перенаправляем пользователя обратно на его профиль
...
async def update_user_field(phone_number: str, field: str, new_value: str):
    async with AsyncSession(engine) as session:
        print(f"Updating field for user with phone number: {phone_number}")  # добавьте эту строку
        # logger.info(f"Updating field for user with phone_number: {phone_number}")  # или это, если у вас настроен логгер
...
# Маршрут к странице конкретного чата
...
async def read_chat(request: Request, chat_id: int, current_user: Union[str, RedirectResponse] = Depends(get_current_user)):
    ...
    return templates.TemplateResponse("chat.html", {"request": request, "chat": chat, "messages": messages, "current_user": current_user, "members": members, "is_member": is_member})  # передача списка участников в шаблон
